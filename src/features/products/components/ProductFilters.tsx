...
export function ProductFilters({ category }: { category?: string }) {
  const { setCategory } = useProductFilters(category);
  ...
  const selectedCount =
    selectedCategories.length + scentFamilies.length + occasions.length;

  const handleCategoryToggle = (id: string) => {
    const nextSelection = selectedCategories.includes(id) ? [] : [id];
    setCategory(nextSelection[0]);
  };
